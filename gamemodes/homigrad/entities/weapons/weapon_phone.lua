...
if SERVER then
	...
	function SWEP:Connect(ply)
		...
		if engine.ActiveGamemode() == "sandbox" and ply:Team() == TEAM_UNASSIGNED then
			plrteam = ply:AccountID()
		else
			plrteam = ply:Team()
		end

		JMod.EZradioEstablish(self, tostring(plrteam)) -- we store team indices as strings because they might be huge (if it's a player's acc id)
		...
		timer.Simple(1, function() if IsValid(self) then self:Speak("Comm line established with J.I. Radio Outpost " .. OutpostID) end end)
	end
			...
			if State == STATE_CONNECTING then
				if self:TryFindSky() then
					...
					if self.ConnectionAttempts > 5 then
						...
						timer.Simple(1, function() if IsValid(self) then self:TurnOff() end end)
					end
				end
			elseif State > 0 then
