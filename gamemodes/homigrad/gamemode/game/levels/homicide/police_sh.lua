...
function CLASS:Event(name, a1)
	...
	if name == "inventory close" then
		if a1.roleT then
			...
			if math.random(1, 2) == 2 then EmitSound(self, "snd_jack_hmcd_cop2.wav")
			else EmitSound(self, "snd_jack_hmcd_cop1.wav") end
		end
	end --]]
end
...
function CLASS:OpenMenu()
	...
	for x, tbl in pairs(interface) do
		x = x - 1

		for y, snd in pairs(tbl) do
			y = y - 1
